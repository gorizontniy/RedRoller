...
```powershell
Copy-Item .\bin\config.example.json .\bin\config.json
```
...
```powershell
python .\bin\yc_ip_hunter.py --config .\bin\config.json --test-telegram
```
...
```powershell
Copy-Item .\bin\telegram_bot_config.example.json .\bin\telegram_bot_config.json
$env:TELEGRAM_BOT_TOKEN="123456:ABCDEF..."
...
`bin\.web-runtime/`; JSON-ключ сервисного аккаунта шифруется перед записью в SQLite
ключом `bin\.web-runtime\secret.key`.
...
шифруется тем же ключом `bin\.web-runtime\secret.key`; API панели не возвращает
